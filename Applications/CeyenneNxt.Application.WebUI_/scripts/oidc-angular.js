...
(function() {
  ...
  var signInCalled = eventPrefix + 'signInCalled';
...
// Module registrarion
      ...
      $routeProvider.when('/auth/callback/:data',
        {
          template: '',
          controller: [
            '$auth', '$routeParams', function($auth, $routeParams) {
  ...
  oidcmodule.factory('oidcHttpInterceptor',
  [
    '$rootScope', '$q', '$auth', 'tokenService', function ($rootScope, $q, $auth, tokenService) {
      return {
        'request': function(request) {

          if (request.url.indexOf($auth.config.apiUrl) !== -1) {
        ...
        'responseError': function(response) {
          ...
          if (response.status == 401) {
            ...
            $rootScope.$broadcast(signInCalled, { response: response });
  ...
  oidcmodule.service('tokenService',
  [
    '$base64', '$localStorage', function($base64, $localStorage) {
      ...
      service.saveAcceessToken = function(accessToken) {
        $localStorage['accessToken'] = accessToken;
        var accessClaims = service.convertToClaims(accessToken);
        $localStorage['access-claims'] = accessClaims;
      };
      ...
      service.hasValidToken = function() {
        if (!this.hasToken() || $localStorage['logoutActive'] === true) return false;
      ...
      service.clearTokens = function() {
        ...
        delete $localStorage['access-claims'];
    ...
    '$routeProvider', function($routeProvider) {
      ...
      var config = {
        ...
        redirectUri: (window.location.origin || window.location.protocol + '//' + window.location.host) +
          ...
          '#/auth/callback/',
        logoutUri: (window.location.origin || window.location.protocol + '//' + window.location.host) +
          window.location.pathname +
          '#/auth/clear',
        state: "",
      ...
      return {
        ...
        // Service itself
        $get: [
          '$q', '$document', '$rootScope', '$localStorage', '$location', 'tokenService',
          function($q, $document, $rootScope, $localStorage, $location, tokenService) {
            ...
            var createLoginUrl = function(nonce, state) {
              ...
              if (config.stickToLastKnownIdp && idpClaimValue) {
                url = url + "&acr_values=" + encodeURIComponent("idp:" + idpClaimValue);
              }
            ...
            var trySilentRefresh = function() {
              ...
              var html = "<iframe src='" +
                url +
                "' height='400' width='100%' id='oauthFrame' style='display:none;visibility:hidden;'></iframe>";
            ...
            var handleSignInCallback = function(data) {
              ...
              if (id_token) {
                ...
                tokenService.saveAcceessToken(fragments['access_token']);
              }
            };
